...
Write a handoff note to a new `.work/handoffs/<YYYY-MM-DD>-<topic>.md` file at the workspace root. Create the `.work/handoffs/` directory if it does not exist and ensure it is listed in `.gitignore` (handoffs are local working notes, not committed history). Use a short kebab-case topic that mirrors the work.
