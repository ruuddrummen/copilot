...
7. **Optionally surface a proposed learning.** If the task surfaced a pattern that could improve the workflow's agents, skills, or instructions, add a `### Proposed Learning` section (1–4 sentences) before the status keyword. The bar is generally applicable across repos — skip or rewrite at a higher abstraction if it names a specific framework/library/runtime/provider, describes this repo's structure, or quotes IDs/paths/commits/branches. Orchestrator decides whether to file it upstream; do not edit skill or instruction files yourself. Most tasks won't warrant one.
