...
7. **Optionally surface a proposed learning.** If you encountered a non-obvious pattern, pitfall, or insight that could improve the workflow's agents, skills, or instructions, include a `### Proposed Learning` section (1–4 sentences) before the status keyword. Only propose learnings that are generally applicable across repositories — not specific to this codebase. **Do not** propose learnings that name a specific framework/library/runtime/provider, describe this repo's organisation, or quote IDs/paths/commits/branches from this repo; rewrite at a higher abstraction level or drop. Do **not** edit skill or instruction files yourself — Orchestrator may offer to file the learning as an upstream issue against the workflow source repo. Most tasks will not warrant one.
